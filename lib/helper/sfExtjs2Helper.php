...
/**
 * @plugin           sfExtjs3Plugin
 * @description      sfExtjs3Plugin is a symfony plugin that provides an easy to use wrapper for the Ext javascript library
 ...
 *                    - Replaced all call_user_func and sfExtjs3Plugin:: by self::
...
class sfExtjs3Plugin {
  ...
  /**
   * Creates an instance of sfExtjs3Plugin.
   *
   * Usage:
   *
   ...
   *                           array
   *                           (
   *                             'adapter' => 'jquery', // config.sf_extjs3_adapters
   *                             'theme'   => 'gray'    // config.sf_extjs3_themes
   *                           ),
   *                           array
   *                           (
  ...
  private function setOptions($options)
  {
    ...
    {
      $this->options[$optName] = isset($options[$optName]) && array_key_exists($options[$optName], sfConfig::get('sf_extjs3_'.$optName.'s', array())) ? $options[$optName] : sfConfig::get('sf_extjs3_default_'.$optName);
    }
  }
   ...
   * config.sf_extjs3_classes then Extjs3.class.constructor will rendered.
   *
   * Usage:
   *
   *   $sfExtjs3Plugin = new sfExtjs3Plugin(
  ...
  /**
   * Creates Javascript source for Extjs3.class
   *
   * Usage:
   *
   *   Syntax A = short form without any options
   *   $sfExtjs3Plugin->Object(array
   *   (
   *     'id'       => 'id',
   *     'renderTo' => $sfExtjs3Plugin->asVar('document.body'),
   *     'items'    => array
   *     (
   *       $sfExtjs3Plugin->Object(array('title'=>'Object A')),
   *       $sfExtjs3Plugin->Object(array('title'=>'Object B'))
   *     )
   *   ));
   *
  ...
  public static function getExtObject($class, $params = array())
  {
    ...
    foreach (sfConfig::get('sf_extjs3_list_attributes') as $attribute)
    {
      if (isset($attributes[$attribute]) && !$attributes[$attribute] instanceof sfExtjs3Var)
      {
  ...
  public function load()
  {
    ...
    $adapters = sfConfig::get('sf_extjs3_adapters', array());

    if ($this->options['adapter']) {
        foreach ($adapters[$this->options['adapter']] as $file)
        {
            $response->addJavascript(sfConfig::get('sf_extjs3_js_dir').$file, 'first');
        }
    }
    ...
    $response->addStylesheet(sfConfig::get('sf_extjs3_css_dir').'ext-all.css', 'first');

    // add css sources for ext fixes
    //$response->addStylesheet(sfConfig::get('sf_extjs3_plugin_dir').'patches/fixes.css', 'first');

    // add css sources for theme
    $themes = sfConfig::get('sf_extjs3_themes', array());

    if ($this->options['theme']) {
        foreach ($themes[$this->options['theme']] as $file)
        {
            $response->addStylesheet(sfConfig::get('sf_extjs3_css_dir').$file, 'first');
        }
    }
  ...
  public function begin($script = true)
  {
    ...
    $source .= self::_comment(sprintf("%s// sfExtjs3Helper: %s%s", self::LBR, sfConfig::get('sf_extjs3_version'), self::LBR));
    $source .= sprintf("Ext.BLANK_IMAGE_URL = '%s'%s", sfConfig::get('sf_extjs3_spacer'), self::LBR_SM);
   ...
   *
   *    $sfExtjs3Plugin->initApplication('App', true, 'myInit');
   *
   ...
   * Usage:
   *
   *     $sfExtjs3Plugin->customClass('Ext.app.symfony.ModuleA', array('title' => 'Module A', 'closable' => false));
   *
   ...
   * Usage:
   *
   *     $sfExtjs3Plugin->asAnonymousClass(array('name'=>'id','mapping'=>'id','type'=>'int'));
   *
   ...
   * Usage:
   *
   *     $sfExtjs3Plugin->asListener(array
   *     (
   *      'rowcontextmenu' => $sfExtjs3Plugin->asMethod(array
   *      (
   ...
   *
   * @param string var
   * @return sfExtjs3Var var
   */
  public static function asVar($var)
  {
    return new sfExtjs3Var($var);
  }
   ...
   *
   *    Syntax A = short form without any options
   *    $sfExtjs3Plugin->asMethod('alert("foo");');
   *
   *     => function() { alert("foo"); }
   *
   *    Syntax B = short form with parameters
   *    $sfExtjs3Plugin->asMethod(array('parameters' => 'msg', 'source' => 'alert(msg)'));
   *
   ...
   *   values that are sfExtjs3Var
   *   values and keys that are listed in sf_extjs3_quote_except
   *
   * @param string key
   * @param string value
  ...
  private static function _quote($key, $value, $always_quote = false)
  {
    if (is_array($value))
    {
      ...
      if (in_array($key, sfConfig::get('sf_extjs3_list_attributes')) && ($key!==0)) //don't know why 0 is a match
      {
    ...
    //always quote has of course an exception for numerics and sfExtjs3Vars
    if (!is_numeric($value) && (($always_quote && !$value instanceof sfExtjs3Var) || (!$value instanceof sfExtjs3Var && self::_quote_except($key, $value))))
    {
      $attribute = '\''.$value.'\'';
      return $attribute;
    }
  ...
  {
    $quoteExcept = sfConfig::get('sf_extjs3_quote_except');
    ...
    $listAttributes = sfConfig::get('sf_extjs3_list_attributes');
  ...
  {
    if (sfConfig::get('sf_extjs3_comment'))
    {
      return $comment;
    }
    else
...
/**
 * @class            sfExtjs3Var
 * @description      sfExtjs3Var is used by quoting logic which ignores variables of this class
